...
<div style="text-align:center">
    共${page.totalPageNum}页/第${page.totalPageNum == 0?0:page.currentPageNum}页
    <c:if test="${page.totalPageNum > 0}">
        ...
        <c:forEach begin="${page.startPage}" end="${page.endPage}" var="pagenum">
            <c:if test="${pagenum != page.currentPageNum}"><a
                    href="${pageContext.request.contextPath}/${page.url}&num=${pagenum}">${pagenum}</a></c:if>
            <c:if test="${pagenum == page.currentPageNum}">${pagenum}</c:if>
        </c:forEach>
